...
const SavedFloorPlans = () => {
    ...
    const fetchUserData = async () => {
        ...
        try {
            ...
            const response = await fetch(`${process.env.REACT_APP_HAPS_MAIN_BASE_URL}floor-plans/all`, {
                method: "POST",
        ...
        <DashboardLayout>
            ...
            {loading ? (
                ...
                <div className="border-solid border-2 black-indigo-600 mt-6">
                    <div className="px-8 mt-5 text-lg font-semibold">Saved Floorplan List</div>
                                    ...
                                    userData.map((user, index) => (
                                        <tr className="bg-white border-b hover:bg-gray-50" key={user.user_id}>
                                            ...
                                            <td
                                                ...
                                                onClick={() => navigate("/saved-floorplans-details", { state: { user } })}
                                            >
                                                {user.user_name}
                                            </td>
                                            <td className="py-4 px-6">{user.user_mobile}</td>
                                            <td className="py-4 px-6">{user.floor_plan_count}</td>
                                            <td className="py-4 px-6 font-semibold">{user.latest_created_at}</td>
                                        </tr>
                                    ))
                                ) : (
                                    <tr>
