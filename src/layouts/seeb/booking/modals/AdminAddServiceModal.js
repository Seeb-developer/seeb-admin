...
const AdminServiceBookingModal = ({
  ...
  return (
    <div className="fixed inset-0 text-black bg-black/50 flex justify-center items-center z-50 !mt-0" onClick={onClose}>
      <div className="bg-white rounded-xl w-[90%] max-w-lg p-6 relative" onClick={(e) => e.stopPropagation()}>
        ...
        <button onClick={onClose} className="absolute top-4 right-4 text-gray-500 hover:text-gray-700 text-xl cursor-pointer">✕</button>
      </div>
    </div>
  );
};
